...
type = st.radio(label="XX", label_visibility="collapsed", 
                  options = ('연도별 변화 추세 분석', '특정 연도의 지자체간 비교분석'), horizontal=True)
#####
st.markdown("---")
if type == '연도별 변화 추세 분석':    
    ...
    st.markdown("---")
    st.markdown("선택하신 변수가 문자형 변수(e.g., 당선자 등)일 경우에는 y축을 확인하시기 바랍니다")
    st.markdown("##")
    metro_data = metro_data[metro_data.CTP_KOR_NM.isin(var7)]
    metro_data = metro_data[metro_data.cat.isin([choice2])]
    #metro_data = metro_data[metro_data.year.isin(range)]
    metro_data = metro_data[(metro_data['year'] >= range[0]) & (metro_data['year'] <= range[1])]
    #print(range)
    #print(metro_data)
    #metro_data["country2"] = metro_data["CTP_KOR_NM"].apply(lambda x: x if x in var7 else '나머지 지역') #country2 만들기     
    #metro_data= metro_data.sort_values(by=['CTP_KOR_NM', 'year'])
    metro_data = metro_data.replace(0, np.nan)                   # 가장 최근연도 데이타가 0으로 되어 있으면 그래프가 이상함
    ...
    #sorted_metro_data = metro_data.copy()                                                               # sort the dataframe
    #sorted_metro_data["order"] = sorted_metro_data["CTP_KOR_NM"].map(countrycode).fillna(len(var7)+1)  # map the value order. var2가 아니면 len(var2)+1
    #sorted_metro_data.sort_values(by=["order", "year"], ascending=False, inplace=True)           # sort by this order

    fig34 = px.line(metro_data, x="year", y="var", color="CTP_KOR_NM", width=550, height=420)                         # sorted된 데이터로 그래프 그리기
    fig34.update_layout(
        xaxis=dict(
    ...
    for i in var7:
        fig34.update_traces(patch={"line":{"color": next(palette), "width":3.3}}, selector={"legendgroup":i})
